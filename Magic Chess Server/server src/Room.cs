...
namespace server
{
	...
	abstract class Room
	{
		...
		protected TcpMessageChannel getMemberAt(int index)
		{
			return _members[index];
		}

		public List<TcpMessageChannel> GetMemberList()
		{
			List<TcpMessageChannel> clone = new List<TcpMessageChannel>();
			_members.ForEach(x => clone.Add(x));
			return clone;
		}
		/**
		...
		*/
		...
		abstract protected void handleNetworkMessage(ISerializable pMessage, TcpMessageChannel pSender);

		/**
		 * Sends a message to all members in the room.
		 */
		protected void sendToAll(ISerializable pMessage)
		{
